 ...
 */
				...
				echo 'function load_images_muploader(){
						jQuery(".mupload_img_holder").each(function(i,v){
							if (jQuery(this).next().next().val() != ""){
								jQuery(this).append("<img src=\"" + jQuery(this).next().next().val() + "\" style=\"height: 150px;width: 600px;\" />");
	    	...
	    	echo '<form method="post" action="" enctype="multipart/form-data">
			    <div class="header_wrap">
					...
					<div style="float:right;margin:32px 0 0 0">
						<input type="submit" style="margin-left: 25px;" value="'.esc_attr(__('Save Changes')).'" name="Submit" class="btn-info btn"><br><br>
					</div>
	...
	public function load_scripts_styles() {
		...
		// @TODO only load styles and js when needed
			...
			wp_enqueue_style( 'Admin_Page_Class', TUR_CSS_URL . 'Admin_Page_Class.css' );
			wp_enqueue_style('iphone_checkbox', TUR_JS_URL. 'iphone-style-checkboxes/style.css');
			
			// Enqueue admin page Scripts
			wp_enqueue_script( 'Admin_Page_Class', TUR_JS_URL . 'Admin_Page_Class.js', array( 'jquery' ), null, true );
			wp_enqueue_script('iphone_checkbox', TUR_JS_URL. 'iphone-style-checkboxes/iphone-style-checkboxes.js',array('jquery'),null,true);
	...
	public function check_field_code() {
		...
		if ( $this->has_field( 'code' ) && $this->is_edit_page() ) {
			...
			wp_enqueue_style( 'at-code-css', TUR_JS_URL .'codemirror/codemirror.css',array(),null);
			wp_enqueue_style( 'at-code-css-dark', TUR_JS_URL .'codemirror/solarizedDark.css',array(),null);
			wp_enqueue_style( 'at-code-css-light', TUR_JS_URL .'codemirror/solarizedLight.css',array(),null);
			wp_enqueue_script('at-code-js', TUR_JS_URL .'codemirror/codemirror.js',array('jquery'),false,true);
			wp_enqueue_script('at-code-js-xml', TUR_JS_URL .'codemirror/xml.js',array('jquery'),false,true);
			wp_enqueue_script('at-code-js-javascript', TUR_JS_URL .'codemirror/javascript.js',array('jquery'),false,true);
			wp_enqueue_script('at-code-js-css', TUR_JS_URL .'codemirror/css.js',array('jquery'),false,true);
			wp_enqueue_script('at-code-js-clike', TUR_JS_URL .'codemirror/clike.js',array('jquery'),false,true);
			wp_enqueue_script('at-code-js-php', TUR_JS_URLh .'codemirror/php.js',array('jquery'),false,true);
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
	...
	public function show_field_image( $field, $meta ) {
		...
		$width = (isset($field['preview_width']))? $field['preview_width'] : '600px';
		if (is_array($meta)){
		...
		if (is_array($meta) && isset($meta['src']) && $meta['src'] != ''){
			...
			$html .= "<input class='at-delete_image_button' type='button' rel='".$field['id']."' value='".esc_attr(__('Delete Image'))."' />";
		}else{
			...
			$html .= "<input class='at-upload_image_button' type='button' rel='".$field['id']."' value='".esc_attr(__('Upload Image'))."' />";
		}
		echo $html;
	}
	...
	public function addPostType($id,$args,$repeater=false){
	...
	$post_types=get_post_types($args,$output); 
	  foreach ($post_types  as $post_type ) {
		...
		$new_field = array('type' => 'select','id'=> $id,'std' => array(),'desc' => '','style' =>'','name' => $name,'multiple' => false,'options' => $options);
		$new_field = array_merge($new_field, $args);
		if(false === $repeater){
			$this->_fields[] = $new_field;
		}else{
	...
	public function addPages($id,$options,$args,$repeater=false){
		$q = array('posts_per_page' => -1, 'post_type'=>'page');
		$temp = array('post_type' =>'page','type'=>'select','args'=>$q);
		$options = array_merge($temp,$options);
		$new_field = array('type' => 'posts','id'=> $id,'desc' => '','name' => 'Posts Field','options'=> $options);
		$new_field = array_merge($new_field, $args);
		if(false === $repeater){
